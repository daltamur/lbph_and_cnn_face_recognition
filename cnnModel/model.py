...
class cnnModel:
    def __init__(self):
        self.model = None
        cnn = input_data(shape=[120,120,1])
    ...
    def train(self, x_train, y_train, x_test, y_test):
        self.model.fit(x_train, y_train, n_epoch=12, validation_set=(x_test, y_test), show_metric=True)
        self.model.save('savedModels/cnn.tfl')

    def predict(self, current_face):
        return self.model.predict(np.array(current_face).reshape(-1, 120, 120, 1))